...
La clase `perceptronSimpleCode` lo que hace es iniciar los valores y las funciones que vamos a usar, la definición `__init__`  que inicializa `self` que es para hacer referencia a un objeto